...
import { escapeXml, niceCeil, round } from './svg.js';
...
export function renderActivity(days, { locale } = {}) {
  ...
  <text x="${PLOT.left - 12}" y="${round(y(v)) + 4}" class="axis" text-anchor="end">${t.formatNumber(v)}</text>`)
    .join('\n');
  ...
  const months = days
    ...
    .map(({ d, i }) => {
      const month = t.months[Number(d.date.slice(5, 7)) - 1];
  ...
  let chart = `  <text x="${WIDTH / 2}" y="${(PLOT.top + PLOT.bottom) / 2}" class="empty muted" text-anchor="middle">${escapeXml(t.activity.empty)}</text>`;
  if (days.length > 0) {
  ...
  const summary = t.activity.summary(t.formatNumber(total), t.formatNumber(peak), total);
  const css = `
  ...
  return card({
    width: WIDTH,
    height: HEIGHT,
    label: `${t.activity.title}: ${summary}`,
    css,
    body: `  <defs>
  ...
  <text x="30" y="44" class="title" style="font-size: 20px">${escapeXml(t.activity.title)}</text>
  <text x="${WIDTH - 30}" y="44" class="summary" text-anchor="end">${escapeXml(summary)}</text>
${grid}
${months}
${chart}`,
  });
