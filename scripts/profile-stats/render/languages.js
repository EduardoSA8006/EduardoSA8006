...
import { escapeXml, formatPercent, round } from './svg.js';
...
export function renderLanguages(languages) {
  ...
  if (languages.length === 0) {
    return card({
      width: 495,
      height: 195,
      label: 'Linguagens mais usadas: sem dados',
      css,
      body: `  <text x="25" y="35" class="title">Linguagens mais usadas</text>
  <text x="247.5" y="115" class="empty muted" text-anchor="middle">Sem dados de linguagens</text>`,
    });
  }
  ...
  <text x="${x + 17}" y="${y}" class="lang">${escapeXml(lang.name)} <tspan class="pct">${formatPercent(lang.percent)}</tspan></text>`;
    })
    .join('\n');

  const summary = languages.map((l) => `${l.name} ${formatPercent(l.percent)}`).join(', ');
  return card({
    width: 495,
    height: 195,
    label: `Linguagens mais usadas: ${summary}`,
    css,
    body: `  <defs>
